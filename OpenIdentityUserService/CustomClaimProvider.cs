...
namespace OpenIdentityCustomService
{
    ...
    public class CustomClaimProvider : IClaimsProvider
    {
        /// <summary>
        /// The logger
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="CustomClaimProvider"/> class.
        /// </summary>
        /// <param name="users">The users service</param>
        public CustomClaimProvider(IUserService users)
        {
            _users = users;
        }
    ...
    {
        public static List<Claim>  GenerateDefaultClaims(SfUserDetail userdetail)
        {
            userdetail.Claims = new List<Claim>();
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.Gender, userdetail.Gender ?? ""));
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.BirthDate,
                userdetail.DateOfBirth == null ? "" : userdetail.DateOfBirth.ToString()));
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.Name, userdetail.FirstName ?? ""));
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.FamilyName, userdetail.LastName ?? ""));
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.Email, userdetail.Email ?? ""));

           // userdetail.Claims.Add(new Claim("res_phone", userdetail.ResPhone ?? ""));
            userdetail.Claims.Add(new Claim(Constants.ClaimTypes.PhoneNumber, userdetail.Mobile ?? ""));
            userdetail.Claims.Add(new Claim("post_code", userdetail.PostCode ?? ""));
            // userdetail.Claims.Add(new Claim(Constants.ClaimTypes.Address, userdetail.StreetAddress1 ?? ""));
            // userdetail.Claims.Add(new Claim("address2", userdetail.StreetAddress2 ?? ""));
            //  userdetail.Claims.Add(new Claim("city", userdetail.City ?? ""));
            userdetail.Claims.Add(new Claim("country", userdetail.CountryCode ?? ""));
            return userdetail.Claims;
        }
    }
}
